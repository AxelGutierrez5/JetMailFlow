...
const url = "https://localhost:7211/api/contacto/register";

export function ModalContact({ titleName, closeModal }) {
  const [useNombre, setNombre] = useState("");
  const [useApellido, setApellido] = useState("");
  const [useEstado, setEstado] = useState("Suscrito");
      ...
      <div className="w-full max-w-lg  overflow-hidden rounded-xl bg-slate-800 border border-slate-700 shadow-2xl transition-all">
        ...
        <form onSubmit={handleOnSubmit}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <select
                    ...
                    className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2.5 pl-10 pr-8 text-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-sm appearance-none cursor-pointer"
                  >
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 border-t border-slate-700 bg-slate-800/50 p-6">
            <button
              onClick={closeModal}
              className="px-4 py-2 bg-transparent border border-slate-600 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg transition-colors text-sm font-medium"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg shadow-lg shadow-indigo-500/30 transition-all text-sm font-medium flex items-center justify-center gap-2"
            >
